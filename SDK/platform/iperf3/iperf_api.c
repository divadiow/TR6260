...
usage()
{
    system_printf(usage_shortstr);
}

void
usage_long(void)
{
    //system_printf(usage_longstr, UDP_RATE / (1024*1024), DURATION, DEFAULT_TCP_BLKSIZE / 1024, DEFAULT_UDP_BLKSIZE);
    system_printf("usage ex:\nserver:iperf -s -4\nudp client:iperf -c 192.168.31.183 -t 10 -u\ntcp client:iperf -c 192.168.31.183 -t 10\n");
}
#endif
                        ...
                        if (test->json_output)
                            ...
                            if (test->role == 's' && !sp->sender) {
                                ...
                                iperf_printf(test, report_bw_format, 
                                    sp->socket, mbuf, (int)start_time, (int)sender_time, ubuf, nbuf, report_sender);
#endif									
                            }
                    }
                } else {
                    ...
                    if (test->json_output) {
                        ...
                        if (test->role == 's' && !sp->sender) {
                            ...
                            iperf_printf(test, report_bw_udp_format, 
                            			sp->socket, mbuf, (int)start_time, (int)sender_time, ubuf, nbuf, 0, 0, (sender_packet_count - sp->omitted_packet_count), 0, report_sender);
#endif
                        }
...
#else
                          iperf_printf(test, report_sum_outoforder, mbuf, (int)start_time, (int)sender_time, (sp->outoforder_packets - sp->omitted_outoforder_packets));
#endif
                    }
                }
...
#else						
                            iperf_printf(test, report_bw_format, sp->socket, mbuf, (int)start_time, (int)receiver_time, ubuf, nbuf, report_receiver);
#endif
                        }
                }
                else {
...
#else
                            iperf_printf(test, report_bw_udp_format, 
                            			sp->socket, mbuf, (int)start_time, (int)receiver_time, ubuf, nbuf, (int)(sp->jitter * 1000.0), (sp->cnt_error - sp->omitted_cnt_error), (receiver_packet_count - sp->omitted_packet_count), (int)lost_percent, report_receiver);
#endif
                        }
                    }
                }
