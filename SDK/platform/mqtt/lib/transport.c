...
{
    transport_list_handle_t list = mqtt_calloc(1, sizeof(struct transport_list_t));
...
int transport_list_add(transport_list_handle_t list, transport_handle_t t, const char *scheme)
{
    ...
    t->scheme = mqtt_calloc(1, strlen(scheme) + 1);
    MEM_CHECK(t->scheme, return -1);
...
{
    transport_handle_t t = mqtt_calloc(1, sizeof(struct transport_item_t));
    MEM_CHECK(t, return NULL);
    return t;
}
