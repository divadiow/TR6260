...
#else
static const uint8_t AIFSN[MAX_AC] = { 1,  1,  1,  1,  1,  1,  7,  3,  2,  2,  1};
static const uint8_t CWMIN[MAX_AC] = { 1,  1,  1,  0,  0,  0,  5,  5,  4,  3,  0};
static const uint8_t CWMAX[MAX_AC] = {2, 2,  2,  1,  0,  0,  10, 10,  5,  4,  0};
#endif
...
typedef struct _LMAC_TXHDR {
    ...
    union {
        struct {
	        ...
	        uint32_t    tid                 : 4;	// Assigned Queue Manager ID
	        uint32_t    tx_count            : 4;
        };
        uint32_t    bd[5];
    };
